...
class Vec3 {
 public:
  ...
  Vec3(const Vec3& v) : m_x{v.x()}, m_y{v.y()}, m_z{v.z()} {}
  ...
  // Prefix
  Vec3& operator++();  //!< Overloaded prefix (++) operator. Returns the vector
                       //!< and increments its components by one.
  Vec3& operator--();  //!< Overloaded prefix (--) operator. Returns the vector
                       //!< and decrements its components by one.
  // Postfix
  Vec3 operator++(int);  //!< Overloaded postfix (++) operator. Increments the
                         //!< vectors components by one and returns it.
  Vec3 operator--(int);  //!< Overloaded postfix (--) operator. Decrements the
                         //!< vectors components by one and returns it.
...
Vec3 operator+(
    const Vec3&,
    const Vec3&);  //!< Overloaded binary (+) operator. Addition of two vectors.
Vec3 operator+(const Vec3&,
               const double&);  //!< Overloaded binary (+) operator. Addition of
                                //!< a vector and a number.
Vec3 operator+(const double&,
               const Vec3&);  //!< Overloaded binary (+) operator. Addition of a
                              //!< vector and a number.

Vec3 operator-(const Vec3&, const Vec3&);  //!< Overloaded binary (-) operator.
                                           //!< Subtraction of two vectors.
Vec3 operator-(const Vec3&,
               const double&);  //!< Overloaded binary (-) operator. Subtraction
                                //!< of a vector and a number.
Vec3 operator-(const double&,
               const Vec3&);  //!< Overloaded binary (-) operator. Subtraction
                              //!< of a vector and a number.
...
Vec3 operator*(const double&,
               const Vec3&);  //!< Overloaded binary (*) operator.
                              //!< Multiplication of a vector and a number.

Vec3 operator/(const Vec3&,
               const Vec3&);  //!< Overloaded binary (/) operator. Division of
...
bool operator==(const Vec3&,
                const Vec3&);  //!< Overloaded binary (==) operator. Checks if
...
std::istream& operator>>(std::istream& in,
                         Vec3& v);  //!< Overloaded binary (>>) operator. Reads
                                    //!< a vector's components from a stream.
std::ostream& operator<<(
    std::ostream&, const Vec3&);  //!< Overloaded binary (<<) operator. Prints a
                                  //!< vector's components to a stream.

double dot(const Vec3&, const Vec3&);  //!< The dot product of two vectors.
                                       //!< Returns a number of type double.
Vec3 cross(const Vec3&,
           const Vec3&);  //!< The cross product of two vectors. Returns a
                          //!< vector perpendicular to the surface of them.

Vec3 getUnitVectorOf(const Vec3&);  //!< Gets the unit vector of a vector
                                    //!< without changing the provided vector.
Vec3 randomVector(
    const double& a = 0.,
    const double& b = 1.);        //!< Returns a vector with uniformly random
